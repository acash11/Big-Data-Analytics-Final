...
class BPlusTree(object):
    ...
    #Will return all key/values pairs within the specified range
    def range_query(self, start, end):
        current_node = self.find(start)
        result = []

        while current_node:
            for i, key in enumerate(current_node.keys):
                if start <= key <= end:
                    result.append((key, current_node.values[i]))
                elif key > end:
                    return result
...
if __name__ == '__main__':
    ...
    bplustree.insert(5, 30)
    bplustree.insert(7, 30)
    bplustree.insert(45, 30)
    bplustree.insert(300, 29)
    bplustree.insert(0, 30)
    bplustree.insert(1, 30)
    bplustree.show()

    a = bplustree.range_query(1, 4000)
    print(a)

    print(bplustree.find(5).next.values)